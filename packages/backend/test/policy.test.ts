...
describe('retired app egress policy', () => {
  it('accepts the empty legacy wire field', async () => {
    const h = await harness();
    const { manifest, digest, body } = policyManifest();
    await pushLive(h, 'usr_ok', manifest, digest, body);
  });

  it('rejects any app controlled outbound destination', async () => {
    const h = await harness();
    const { manifest } = policyManifest({
      egress: { allowedHosts: ['api.stripe.com'], credentials: [] },
    });
    await expect(h.platform.for('usr_x').sync({ identity: ident(), manifest })).rejects.toThrow(
      /egress policy is no longer supported/,
    );
  });
});
