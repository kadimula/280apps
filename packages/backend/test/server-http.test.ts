...
describe('real node server', () => {
  ...
  it('completes a push and parks on a missing declared integration binding', async () => {
    ...
    expect(st.state).toBe('waiting_secrets');
    expect(st.integrationNotice).toContain('todos (google-sheets)');
