...
- Gateway (identity front door, built not yet deployed) = `packages/gateway` (npm `@280/gateway`): the edge Worker on `*.280apps.run` that authenticates a viewer (Google + Microsoft Entra OIDC via `@280/backend/auth/oidc`, sessions via reused `Auth`), then mints a short-lived ES256-signed `X-280-Identity` header app code verifies offline. Reuses `authsvc`/`oidc` (relocated, not forked; the only new Auth config is an injected `resolveRedirect` guard). Grows `platform/dispatcher` (supersedes it on the route). Access check (`access.ts` `AllowAllAccess`) and the real proxy (`upstream.ts` `StubUpstream`) are marked seams for the trailing grants/container tasks. Signed-header scheme (custody, TTL, claims, verification): `packages/gateway/README.md`.
