...
// that runs the real start -> callback handshake to hand a test a session cookie.
// Nothing reaches the network; the login identity is encoded in the "code" the
// test supplies.
...
// maps a login code straight to an identity, so a test signs in as anyone by
// passing their email as the code. The code "boom" fails the exchange, which
// exercises the failed-login path.
...
// builds an Auth with the fake provider and test origins; overrides let a case
// tighten the rate limit or pin the clock.
...
// returns the "name=value" a Set-Cookie header carries, verbatim, so it can be
// echoed back on the next request without re-encoding.
...
// drives the real handshake against the fake provider and returns the session
// cookie pair. redirect exercises the redirect whitelist; callers rarely care.
