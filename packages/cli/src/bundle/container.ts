...
// A Next.js instrumentation hook 280 injects so a production error's digest is
// logged next to its stack in one line: Next.js otherwise ships the digest to the
// browser but logs only the stack, leaving `two80 logs --digest <id>` nothing to
// match. Emitted as .js so it compiles in both TS and plain-JS apps.
const INSTRUMENTATION_FILE = 'instrumentation.js';
const INSTRUMENTATION = `// Generated by 280. Logs each production error's digest with its stack as one
// structured line so \`two80 logs --digest <id>\` can resolve it to a real stack.
export function onRequestError(err, request, context) {
  try {
    ...
    // Error reporting must never itself throw inside the error path.
  }
}
`;
// The instrumentation filenames Next.js recognizes, at the project root or (when the
// app uses a src directory) under src/. If the app ships any of these, 280 leaves
// error reporting to the user and injects nothing.
...
// injectInstrumentation adds the 280 onRequestError hook to the build context, but
// only when the app ships no instrumentation file of its own — a user file is never
// clobbered. The hook is placed where Next.js reads it: under src/ when the app uses
// a src directory, else at the root.
